...
public class StocksManager { 
	...
	public void editStocks() { //주식'종목 정보' 수정하는 함수 생성
		...
		for(int i = 0; i<stocks.size(); i++) { //name배열에 입력된 크기만큼 for반복문 실행하게 loop설계
			StockInput stockInput = stocks.get(i); //stocks저장공간에 있는 인덱스 값을 stock에 할당
			if(stockInput.getStocksName().equals(name)) { //만약 입력값과 현재 저장되어있는 주식종목 명이 값을 경우 문자열이므로 equals() method를 활용 
